...
fun ComicPageRead(
    ...
    HorizontalPager(
        ...
        val reverse = localSetting.readMode == "pageReverse"
        if (isLargeScreen()) {
            // 大屏（平板横屏）：左右并排双页，每页显示当前图与下一图，避免单图被放大成全屏巨图
            val leftPage = if (!reverse) page else page - 1
            val rightPage = if (!reverse) page + 1 else page
            Row(modifier = Modifier.fillMaxSize()) {
                ...
                Box(
                    ...
                    if (leftPage in list.indices) {
                        ComicPicImage(
                            comicPicImageState = list[leftPage],
                ...
                Box(
                    ...
                    if (rightPage in list.indices) {
                        ComicPicImage(
                            comicPicImageState = list[rightPage],
                            modifier = Modifier.fillMaxSize(),
                            contentScale = ContentScale.Fit,
                            onClickLeft = {
                                comicReadViewModel.prev(context)
